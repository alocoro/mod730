...
class Comune_Mapper_Residente extends Comune_Mapper_Abstract {
	private static $PARAM_MAPPER = array('cf' => 'cf',
										 'nome' => 'nome',
										 'cognome' => 'cognome',
										 'sesso' => 'sesso',
	...
	public function __construct() {
		$this->table_name = 'Comune_Model_DbTable_Residente';
	}
	...
	public function find($cf, $params = array()) {
		...
		$nucleo_familiare_mapper = new Comune_Mapper_NucleoFamiliareElemento();
		$nucleo_familiare = new Comune_Model_NucleoFamiliare($nucleo_familiare_mapper->find($row->cf));

		return new Comune_Model_Residente($row->cf, $row->nome, $row->cognome,
										  $row->sesso, $row->data_nascita,
										  $row->comune_nascita, $row->provincia_nascita,
										  $row->stato_civile, $row->comune_residenza,
										  $row->provincia_residenza, $row->cap_residenza,
										  $row->tipologia_residenza, $row->indirizzo_residenza,
										  $row->numcivico_residenza, $row->frazione_residenza,
										  $row->ultima_variazione_residenza,
										  $nucleo_familiare);
	}
